...
- Current packaged artifact name: `logs-viewer_0.1.3.zip`
- Release state:
...
|---------|------|--------|------------------|
| `0.1.3` | 2026-03-02 | Stable | Hotfix release for the public-first web image pipeline: corrected `web/Dockerfile.same-origin` to copy Vite output from `/app/resources/web`, resolving GHCR publish workflow failures on `main` and release tags |
...
- Next cut should bump `app.json` to `0.1.4` (or `0.2.0` if scope expands), then regenerate package/tag.

## 4. Next version recommendation

Recommended next release when scope is accepted:

- Suggested version: `0.1.4` (patch) for incremental fixes, or `0.2.0` for new feature scope.
- Rationale:
